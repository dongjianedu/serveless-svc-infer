...
from azure_tts import get_azure_wav, delete_azure_wav
import datetime
...
def upload_wav(job_id, image_location, result_index=0, results_list=None): # pragma: no cover
    '''
...
def main(job):
   ...
   #-m logs/44k/G_125600.pth -c logs/44k/config.json -n outputaudio.wav -s zhanyin -f0p rmvpe -d cpu -sd -30

    # 一定要设置的部分
    parser.add_argument('-m', '--model_path', type=str, default="logs/44k/G_125600.pth", help='模型路径')
    ...
    parser.add_argument('-s', '--spk_list', type=str, nargs='+', default=['zhanyin'], help='合成目标说话人名称')
    ...
    parser.add_argument('-d', '--device', type=str, default='cpu', help='推理设备，None则为自动选择cpu和gpu')
    ...
    # azure tts logic
    parser.add_argument('-af', '--audio_file', type=str, default="azure", help='音频文件前缀')
    ...
    txt = job_input.get('promote')

    get_azure_wav(audio_file, txt, "/tmp/", speaker, style)


    clean_names = args.clean_names
    clean_names = [audio_file]
    trans = args.trans
    ...
    for clean_name, tran in zip(clean_names, trans):
        raw_audio_path = f"/tmp/{clean_name}"
        ...
        for spk in spk_list:
            ...
            if use_spk_mix:
                spk = "spk_mix"
            res_path = f'/tmp/{clean_name}_{key}_{spk}{cluster_name}_{isdiffusion}_{f0p}.{wav_format}'
            soundfile.write(res_path, audio, svc_model.target_sample, format=wav_format)
            svc_model.clear_empty()
    wav_url = upload_wav(job['id'], res_path)
    #delete_azure_wav(audio_file, "./raw/")
    #os.remove(res_path)
    return wav_url
